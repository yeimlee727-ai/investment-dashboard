...
- 실제 주문 없는 모의매매
- CSV/Excel 파일 기반 MockBroker 가상 포지션 일괄 등록
...
- 프리미엄 다크 UI: 카드형 KPI, 정돈된 상태 배지, 한글 중심 표/차트, 통일된 다크 차트 스타일
...
전체 화면은 공통 다크 테마와 카드형 정보 구조를 사용합니다.

- 메인 대시보드는 앱 상태, 데이터 모드, 시스템 안전 상태, 포트폴리오 KPI, 주요 분석 요약을 카드 형태로 보여줍니다.
- 표는 사용자 표시용 한글 컬럼명과 안전한 숫자 포맷을 사용하며, 계산 불가 값은 `계산 불가` 또는 `-`로 표시합니다.
- Plotly 차트는 다크 배경, 정돈된 범례, 최소화된 grid line을 적용해 화면 간 시각 스타일을 맞춥니다.
- SAMPLE/FALLBACK, 실제 주문 없음, MockBroker, 백테스트/전략분석/리스크 한계 안내는 유지하되 화면을 덜 압도하도록 상태 배지와 안내 박스로 정리했습니다.
- 이번 UI 개선은 표시 계층 중심이며 기존 백테스트, 리스크, 전략분석, MockBroker 계산 로직은 유지합니다.
- 실제 주문, 실제 계좌 조회, 실제 증권사 API 주문 호출 기능은 여전히 없습니다.
...
모의매매 페이지의 `보유종목 일괄 업로드` 섹션에서 CSV 또는 `.xlsx` 파일을 업로드해 MockBroker 가상 포지션을 한 번에 등록할 수 있습니다.

- 이 기능은 파일 기반 MockBroker 가상 포지션 등록 기능이며 실제 주문, 실제 체결, 실제 계좌 조회, 외부 서비스 연결과 무관합니다.
- 필수 컬럼은 `symbol/종목코드`, `market/시장`, `quantity/수량`, `avg_price/평균단가`입니다.
- 선택 컬럼으로 `name/종목명`, `currency/통화`, `memo/메모`, `sector/섹터`, `current_price/현재가`, `account/계좌`, `source/출처`를 사용할 수 있습니다.
- 한글/영문 컬럼명은 가능한 범위에서 자동 매핑합니다. 예: `종목코드`, `코드`, `symbol`, `ticker`는 모두 `symbol`로 해석합니다.
- 시장값은 `국내`, `한국`, `KR`, `KRX`, `KOSPI`, `KOSDAQ`을 `KR`로, `미국`, `해외`, `US`, `NASDAQ`, `NYSE`, `AMEX`를 `US`로 표준화합니다.
- KR 종목 평균단가는 KRW 기준, US 종목 평균단가는 USD 기준으로 입력해야 합니다.
- 반영 방식은 `기존 포지션 유지 + 추가/업데이트`, `기존 동일 종목만 덮어쓰기`, `전체 MockBroker 포지션 초기화 후 교체` 중 선택할 수 있습니다.
- 업로드 전 빈 파일, 필수 컬럼 누락, 잘못된 시장구분, 0 이하 수량/평균단가, NaN/inf, 파일 내 중복 행을 검증합니다.
- 파일을 읽으면 파일명과 인식 행 수가 표시되고, 검증 후 `반영 가능/오류/경고` 건수 요약이 표시됩니다.
- 경고만 있는 행은 사용자가 확인 체크박스를 선택해야 반영할 수 있습니다. 오류가 있는 행은 반영하지 않습니다.
- 반영 후에는 신규 추가, 업데이트, 건너뜀, 오류 제외 건수와 현재 MockBroker 가상 포지션 수가 표시됩니다.
- 반영 결과 상세 표에서 종목별 이전 수량/평균단가와 신규 수량/평균단가를 확인할 수 있습니다.
- `전체 MockBroker 포지션 초기화 후 교체`는 로컬 MockBroker 가상 포지션만 교체하며 실제 계좌와 무관합니다.
- 업로드 파일의 `current_price`는 참고 입력값이며 실제 현재가나 실제 체결가를 보장하지 않습니다.
- SAMPLE/FALLBACK 데이터와 환율 오류가 있으면 업로드 후 원화 환산 비중과 손익이 실제와 다를 수 있습니다.
...
- 통합 포트폴리오 비중은 `market_value_krw` 기준으로 계산하며, US 종목의 원통화 `market_value`를 KR 종목 원화 값과 직접 합산하지 않습니다.
- 환율은 `SAMPLE` 모드에서는 샘플 환율을 사용하고, `REAL_WITH_FALLBACK` 모드에서는 외부 조회를 시도한 뒤 실패하면 fallback 환율을 표시합니다.
- SAMPLE 모드에서도 US 종목 가격은 USD 단위 샘플 가격으로 생성되며, GRAB 같은 저가 미국 종목이 KRW 규모 가격으로 표시되지 않도록 보정합니다.
...
메인 대시보드의 `리포트 다운로드` 섹션에서 현재 MockBroker 가상 포지션과 앱 내부 분석 결과를 포트폴리오 점검 리포트로 저장할 수 있습니다.

- Excel 리포트는 `Summary`, `Positions`, `Strategy`, `Risk_Rebalancing`, `Stress_Test`, `Allocation`, `Limitations` 시트로 구성됩니다.
- HTML 리포트는 브라우저에서 열어 인쇄 기능으로 PDF 저장할 수 있습니다. 한글 폰트와 실행 환경 차이를 줄이기 위해 기본 제공 형식은 Excel과 HTML입니다.
- 리포트에는 생성일시, 데이터 모드, 환율 상태, 포트폴리오 요약, 모의매매 포지션 평가, 전략분석, 리스크·리밸런싱, 스트레스 테스트, 추가 투자금 배분안, 제한사항이 포함됩니다.
- 리포트는 MockBroker 가상 포지션과 앱 분석 결과 기반의 의사결정 보조 정보이며 투자 추천이나 실제 주문 실행 기능이 아닙니다.
- 실제 주문, 실제 체결, 실제 계좌 조회, 외부 증권사 연동은 수행하지 않습니다.
- SAMPLE/FALLBACK 데이터, 외부 현재가, 환율, 세금, 수수료, 슬리피지는 실제 시장 및 증권사 평가금액과 다를 수 있습니다.
